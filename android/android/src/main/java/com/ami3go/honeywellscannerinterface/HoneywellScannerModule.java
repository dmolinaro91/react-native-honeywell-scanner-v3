...
public class HoneywellScannerModule extends ReactContextBaseJavaModule implements BarcodeReader.BarcodeListener {
    ...
    public void startReader(final Promise promise) {
        // Si ya hay un reader activo, no crear uno nuevo
        if (isReaderActive && reader != null) {
            Log.w(HoneyWellTAG, "Reader already active, skipping initialization");
            ...
            public void onCreated(AidcManager aidcManager) {
                ...
                if (reader != null) {
                    ...
                    try {
                        ...
                        Log.e(HoneyWellTAG, "Error setting scanner properties: " + e.getMessage());
                        isReaderActive = false;
                        promise.resolve(false);
                        e.printStackTrace();
                    } catch (IllegalStateException e) {
                        Log.e(HoneyWellTAG, "BarcodeReader is closed or in invalid state: " + e.getMessage());
                        isReaderActive = false;
                        promise.resolve(false);
                        e.printStackTrace();
                    } catch (Exception e) {
                        Log.e(HoneyWellTAG, "Unexpected error starting reader: " + e.getMessage());
                        isReaderActive = false;
                        promise.resolve(false);
                        e.printStackTrace();
                    }
                } else {
                    Log.e(HoneyWellTAG, "Failed to create BarcodeReader");
                    promise.resolve(false);
                }
            }
        });
    ...
    public void stopReader(Promise promise) {
        try {
                ...
                Log.e(HoneyWellTAG, "Error in stopReader: " + e.getMessage());
                isReaderActive = false;
                promise.resolve(null); // Resolver de todas formas para no bloquear JS
            }
    }
